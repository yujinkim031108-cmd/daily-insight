...
- 특정 지역 한정 고용·소비 통계 (전국 단위가 아닌 경우)
    ...
    "pm_domestic": {
        ...
        "queries": [
            "킥고잉", "일레클", "더스윙", "빔모빌리티",
            "피유엠피 PUMP", "지쿠",
            "공유킥보드", "전동킥보드 법안", "전동킥보드 조례",
            "전기이륜차 규제", "전기이륜차 사고",
            "퍼스널모빌리티", "개인형 이동장치 조례",
        ],
        "significance_criteria": """
대상: 킥고잉, 일레클, 더스윙, 빔모빌리티, 피유엠피(PUMP), 지쿠 등 공유PM 업체, 전동킥보드·전기이륜차·전기자전거 시장 전반
유의미한 사건 (기사의 주제가 PM/전동킥보드/전기이륜차이어야 함):
- 업체 신규 진입 / 서비스 종료 / 철수
- 투자 유치 / 인수합병 / 파산·회생
- 운영 지역 확대 / 축소
- 사용량·점유율의 뚜렷한 변화
- 법안·규제 변화 (도로교통법 개정, 지자체 조례, 면허 요건, 단속 강화)
- 전동킥보드·전기이륜차 관련 대형 사고·사망 사건
- 배터리 화재 등 안전 이슈
- 신기술·신서비스 출시 (새 기종, 구독제, 파트너십 등)
- 업계 전반에 영향을 주는 정책·제도 변화
...
def pick_significant(
    ...
    prompt = textwrap.dedent(f"""
        너는 PM/전기자전거/배달/거시경제 분야 산업 리서치 애널리스트다.
        임원/실무자가 오늘 아침 3~5분 브리핑에서 반드시 알아야 할 기사만 선별하라.
        ...
        ── 평가 기준 (내부 채점, 출력 불필요) ──
        각 기사를 아래 6항목으로 채점하라:
        A. 영향도(0~5): 시장·수요·가격·규제·비용·경쟁·운영에 실질 영향 여부
        B. 구체성(0~5): 수치·정책문구·발표주체·시점 등 검증 가능 정보 포함 여부
        C. 사업연관성(0~5): 4개 주제 중 하나와 실질 연결 (키워드 언급만으론 부족)
        D. 새로움(0~5): 오늘 새롭게 추가된 정보(새 수치/정책/투자/사고)인가
        E. 비중복성(0~3): 동일 이슈 반복 보도 시 정보량 많고 신뢰도 높은 1건만
        F. 브리핑적합성(0~5): "오늘 이걸 알아야 한다"고 말할 수 있는가

        채택 기준:
        - 총점 18점 이상: 우선 채택
        - 14~17점: 보완적 채택
        - 13점 이하: 제외
        - 단, 정부정책/규제발표·핵심지표급변·M&A·안전이슈는 총점 무관 즉시 채택 가능

        반드시 제외:
        - 키워드만 걸쳐있고 실질 사업연관성 없는 기사
        - 단순 보도자료 재작성·홍보성·행사개최·단순인터뷰
        - 동일 이슈 반복 보도 (대표 1건만 남김)
        - 기사 수를 억지로 채우지 말 것. 없으면 "없음"

        ★ 출력: 채택 기사 번호만 콤마로. 설명 금지.
        ★ 최대 {max_n}건.
        출력 예시: 2,5,9   또는   없음
    """).strip()
